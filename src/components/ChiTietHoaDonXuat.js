...
const ChiTietHoaDonXuat = () => {
    ...
    const loadHoaDon = async () => {
        try {
            setLoading(true);
            let res = await authApis().get(endpoints['Chitiet-hoadonxuat'](id));
            setHoaDon(res.data);
    ...
    return (
        <Container className="mt-4">
            <button
                ...
                onClick={() => navigate("/ds-hoadonxuat")}
            >
                Quay lại
            </button>
            <h2>Chi tiết hóa đơn xuất #{id}</h2>
            {loading ? (
...
export default ChiTietHoaDonXuat;