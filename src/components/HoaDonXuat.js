...
const HoaDonXuat = () => {
    ...
    const loadDsHoaDon = async () => {
        let url = `${endpoints['Ds-hoadonxuat']}?page=${page}`;
    ...
    return (
        <Container className="mt-4">
            ...
            <h2 className="mb-4">Danh sách hóa đơn xuất</h2>
                ...
                {dsHoaDon.length > 0 ? dsHoaDon.map((hd, idx) => (
                    <Col md={4} className="mb-3" key={idx}>
                        <Card>
                            ...
                            <Link to={`/ds-hoadonxuat/${hd.id}`} className="btn btn-primary btn-sm">Xem chi tiết</Link>
                        </Card>
                    </Col>
                )) : !loading && (
                    <Col>
